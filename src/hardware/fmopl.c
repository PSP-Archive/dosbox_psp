 ...
 *
 * Supposedly faster OPL emulator from scummvm
 */
...
#ifdef PSP
	return __builtin_allegrex_max(__builtin_allegrex_min(val, max), min);
#else
	if ( val > max )
		val = max;
	else if ( val < min )
...
static int OPLOpenTable(void) {
	...
	for (t = 0; t < EG_ENT - 1 ; t++) {
		rate = ((1 << TL_BITS) - 1) / powf(10.0, EG_STEP * t / 20);	/* dB -> voltage */
		TL_TABLE[         t] =  (int)rate;
	...
	for (s = 1;s <= SIN_ENT / 4; s++) {
		pom = sinf(2 * PI * s / SIN_ENT); /* sin     */
		pom = 20 * log10f(1 / pom);	   /* decibel */
	...
	for (i=0; i < EG_ENT; i++) {
		/* ATTACK curve */
		pom = powf(((float)(EG_ENT - 1 - i) / EG_ENT), 8) * EG_ENT;
	...
	/* make LFO ams table */
	for (i=0; i < AMS_ENT; i++) {
		pom = (1.0 + sinf(2 * PI * i / AMS_ENT)) / 2; /* sin */
	...
	for (i=0; i < VIB_ENT; i++) {
		/* 100cent = 1seminote = 6% ?? */
		pom = (float)VIB_RATE * 0.06 * sinf(2 * PI * i / VIB_ENT); /* +-100sect step */
...
/* ---------- update one of chip ----------- */
static void YM3812UpdateOne(FM_OPL *OPL, int16 *buffer, int length) {
	int i;
	int data;
	int16 *buf = buffer;
...
static int OPLTimerOver(FM_OPL *OPL, int c) {
	...
	/* reload timer */
//	if (OPL->TimerHandler) (OPL->TimerHandler)(OPL->TimerParam + c, (float)OPL->T[c] * OPL->TimerBase);
	return OPL->status >> 7;
}
